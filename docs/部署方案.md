# 🚀 部署方案指南

由于 Vercel 免费计划存在限制（函数执行时间、带宽、部署次数等），本文档提供多种替代部署方案。

## 📋 Vercel 限制说明

### Hobby（免费）套餐限制
- **函数执行时间**：最多 10 秒
- **函数数量**：最多 12 个 Serverless Functions
- **带宽**：100GB/月
- **部署次数**：无限制（但可能因其他限制导致失败）
- **文件大小**：单文件最大 50MB

### 常见问题
1. **函数超时**：AI 接口调用可能超过 10 秒
2. **带宽耗尽**：大量静态资源访问
3. **冷启动慢**：Serverless 函数首次调用延迟

---

## 🎯 推荐替代方案

### 方案 1：Railway（推荐）⭐

**优点**：
- 免费额度：$5/月（足够小项目使用）
- 支持 Flask 应用
- 自动部署
- 无函数执行时间限制
- 国内访问速度较好

**部署步骤**：

1. **注册 Railway**
   - 访问 https://railway.app
   - 使用 GitHub 账号登录

2. **创建新项目**
   ```bash
   # 安装 Railway CLI
   npm i -g @railway/cli
   
   # 登录
   railway login
   
   # 初始化项目
   railway init
   ```

3. **配置环境变量**
   - 在 Railway Dashboard 中设置所有环境变量
   - 参考 `.env.example` 或 `config.example.js`

4. **创建 `Procfile`**
   ```bash
   web: gunicorn app:app --bind 0.0.0.0:$PORT
   ```

5. **创建 `runtime.txt`**（指定 Python 版本）
   ```
   python-3.11
   ```

6. **部署**
   ```bash
   railway up
   ```

---

### 方案 2：Render

**优点**：
- 免费套餐可用（有休眠限制）
- 支持 Flask
- 自动部署
- 简单易用

**部署步骤**：

1. **注册 Render**
   - 访问 https://render.com
   - 使用 GitHub 账号登录

2. **创建 Web Service**
   - 连接 GitHub 仓库
   - 选择项目根目录

3. **配置**
   - **Build Command**: `pip install -r requirements_server.txt`
   - **Start Command**: `gunicorn app:app`
   - **Environment**: Python 3

4. **设置环境变量**
   - 在 Environment Variables 中添加所有需要的变量

5. **部署**
   - 点击 "Create Web Service" 自动部署

---

### 方案 3：Fly.io

**优点**：
- 免费额度：3 个共享 CPU 实例
- 全球边缘部署
- 支持 Docker

**部署步骤**：

1. **安装 Fly CLI**
   ```bash
   # Windows (PowerShell)
   iwr https://fly.io/install.ps1 -useb | iex
   
   # Mac/Linux
   curl -L https://fly.io/install.sh | sh
   ```

2. **登录**
   ```bash
   fly auth login
   ```

3. **初始化**
   ```bash
   fly launch
   ```

4. **配置 `fly.toml`**
   ```toml
   app = "your-app-name"
   primary_region = "hkg"  # 香港，国内访问快
   
   [build]
   
   [http_service]
     internal_port = 5000
     force_https = true
     auto_stop_machines = true
     auto_start_machines = true
     min_machines_running = 0
   
   [[services]]
     processes = ["app"]
     http_checks = []
     internal_port = 5000
     protocol = "tcp"
   ```

5. **部署**
   ```bash
   fly deploy
   ```

---

### 方案 4：国内云服务器（推荐国内用户）⭐

**优点**：
- 完全控制
- 无限制
- 国内访问速度快
- 成本可控（学生优惠）

**推荐平台**：
- **阿里云**：学生优惠 9.5 元/月
- **腾讯云**：学生优惠 10 元/月
- **华为云**：学生优惠
- **UCloud**：价格便宜

**部署步骤**：

1. **购买服务器**
   - 选择 Ubuntu 20.04 或 22.04
   - 最低配置：1核2G（足够使用）

2. **服务器配置**
   ```bash
   # 更新系统
   sudo apt update && sudo apt upgrade -y
   
   # 安装 Python 3.11
   sudo apt install python3.11 python3.11-venv python3-pip nginx -y
   
   # 安装 Node.js（如果需要）
   curl -fsSL https://deb.nodesource.com/setup_18.x | sudo -E bash -
   sudo apt install -y nodejs
   ```

3. **部署应用**
   ```bash
   # 克隆项目
   cd /var/www
   git clone <your-repo-url> AI_LL
   cd AI_LL
   
   # 创建虚拟环境
   python3.11 -m venv venv
   source venv/bin/activate
   
   # 安装依赖
   pip install -r requirements_server.txt
   
   # 配置环境变量
   cp .env.example .env
   nano .env  # 编辑环境变量
   ```

4. **配置 Gunicorn**
   ```bash
   # 创建 systemd 服务
   sudo nano /etc/systemd/system/french-ai.service
   ```
   
   内容：
   ```ini
   [Unit]
   Description=French AI Learning Hub
   After=network.target
   
   [Service]
   User=www-data
   Group=www-data
   WorkingDirectory=/var/www/AI_LL
   Environment="PATH=/var/www/AI_LL/venv/bin"
   ExecStart=/var/www/AI_LL/venv/bin/gunicorn app:app --bind 127.0.0.1:5000 --workers 2
   
   [Install]
   WantedBy=multi-user.target
   ```
   
   ```bash
   # 启动服务
   sudo systemctl daemon-reload
   sudo systemctl enable french-ai
   sudo systemctl start french-ai
   ```

5. **配置 Nginx**
   ```bash
   sudo nano /etc/nginx/sites-available/french-ai
   ```
   
   内容：
   ```nginx
   server {
       listen 80;
       server_name your-domain.com;  # 或服务器IP
       
       # 静态文件
       location / {
           root /var/www/AI_LL;
           try_files $uri $uri/ /index.html;
       }
       
       location /public {
           alias /var/www/AI_LL/public;
       }
       
       # API 代理
       location /api {
           proxy_pass http://127.0.0.1:5000;
           proxy_set_header Host $host;
           proxy_set_header X-Real-IP $remote_addr;
           proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
           proxy_set_header X-Forwarded-Proto $scheme;
       }
   }
   ```
   
   ```bash
   # 启用配置
   sudo ln -s /etc/nginx/sites-available/french-ai /etc/nginx/sites-enabled/
   sudo nginx -t
   sudo systemctl restart nginx
   ```

6. **配置 SSL（可选但推荐）**
   ```bash
   sudo apt install certbot python3-certbot-nginx
   sudo certbot --nginx -d your-domain.com
   ```

---

### 方案 5：Docker + 任意平台

**优点**：
- 环境一致
- 易于迁移
- 支持多种平台

**创建 Dockerfile**：
```dockerfile
FROM python:3.11-slim

WORKDIR /app

# 安装依赖
COPY requirements_server.txt .
RUN pip install --no-cache-dir -r requirements_server.txt

# 复制应用
COPY . .

# 暴露端口
EXPOSE 5000

# 启动命令
CMD ["gunicorn", "app:app", "--bind", "0.0.0.0:5000", "--workers", "2"]
```

**部署到任意支持 Docker 的平台**：
- Railway
- Render
- Fly.io
- 云服务器
- Docker Hub + 云服务

---

## 🔧 迁移检查清单

### 前端配置修改

如果后端 API 地址改变，需要修改前端配置：

1. **修改 `scripts/services/apiService.js`**
   ```javascript
   // 从 Vercel 环境变量或配置中读取
   const API_BASE_URL = process.env.API_BASE_URL || 'https://your-new-backend.com';
   ```

2. **或者使用环境变量**
   - 在部署平台设置 `API_BASE_URL`
   - 前端通过配置读取

### 环境变量迁移

确保以下环境变量在新平台正确配置：

- `SUPABASE_URL`
- `SUPABASE_SECRET_KEY` 或 `SUPABASE_SERVICE_ROLE_KEY`
- `COZE_BOT_ID`
- `COZE_PAT_TOKEN` 或 `COZE_SAT_TOKEN`
- `DEEPSEEK_API_KEY`
- `TMDB_API_KEY`
- `JWT_SECRET`（如果使用）

### 静态文件处理

如果静态文件较大（如词典文件），建议：

1. **使用 CDN**（如 Cloudflare、阿里云 OSS）
2. **使用 Supabase Storage**（推荐，已在项目中支持）
3. **使用对象存储服务**（AWS S3、阿里云 OSS、腾讯云 COS）

---

## 📊 方案对比

| 方案 | 成本 | 国内访问 | 易用性 | 限制 | 推荐度 |
|------|------|----------|--------|------|--------|
| Railway | $5/月免费额度 | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ | 无 | ⭐⭐⭐⭐⭐ |
| Render | 免费（有休眠） | ⭐⭐ | ⭐⭐⭐⭐ | 休眠限制 | ⭐⭐⭐⭐ |
| Fly.io | 免费额度 | ⭐⭐⭐ | ⭐⭐⭐ | 3个实例 | ⭐⭐⭐⭐ |
| 国内云服务器 | 10-50元/月 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | 无 | ⭐⭐⭐⭐⭐ |
| Docker | 取决于平台 | 取决于平台 | ⭐⭐⭐ | 取决于平台 | ⭐⭐⭐⭐ |

---

## 🚨 常见问题

### Q: 如何测试 Flask 应用是否正常？
```bash
# 本地测试
python app.py

# 或使用 gunicorn
gunicorn app:app --bind 127.0.0.1:5000
```

### Q: 静态文件无法访问？
- 确保 Nginx 配置正确
- 检查文件权限：`sudo chown -R www-data:www-data /var/www/AI_LL`

### Q: API 返回 404？
- 检查路由配置
- 确保 Flask 应用正确处理 `/api/*` 路径

### Q: CORS 错误？
- 确保 Flask-CORS 已安装并配置
- 检查 `app.py` 中的 CORS 设置

---

## 📝 下一步

1. **选择适合的方案**（推荐 Railway 或国内云服务器）
2. **按照步骤部署**
3. **测试所有功能**
4. **更新前端 API 地址**
5. **配置域名和 SSL**

---

**最后更新**：2024-12-19

